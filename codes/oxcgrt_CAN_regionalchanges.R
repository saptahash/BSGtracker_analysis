...
latest_date <- lubridate::ymd(max(oxcgrt_can$Date))

#finding earliest date - data from last 3 weeks
earliest_date <- lubridate::ymd(latest_date) - 36 

#subsetting only the time period that's needed - i.e. 3 weeks
# Filtering subnational data
oxcgrt_regionalchanges <- oxcgrt_can %>% filter(!(RegionName == ""))

# extracting the columns of indicator values specifically
indicators <- str_subset(names(oxcgrt_can), pattern = "(C|H|E)[0-9][_]")
...
agg_tibble <- agg_tibble %>% filter(lubridate::ymd(Date) >= earliest_date)

agg_tibble <- agg_tibble %>% mutate(Date = format(ymd(Date), "%d-%b-%y"))

write.csv(agg_tibble, "./data/OxCGRT_CAN_changes.csv")
